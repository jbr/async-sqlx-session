...
///
/// ```rust
/// use async_sqlx_session::{SqliteSessionStore, Error};
    ...
    async fn load_session(&self, cookie_value: String) -> Result<Option<Session>, Self::Error> {
        ...
        let result: Option<(String, Option<OffsetDateTime>)> =
            sqlx::query_as(&self.substitute_table_name(
                r#"
            SELECT session, expires FROM %%TABLE_NAME%%
...
mod tests {
    ...
    async fn updating_a_session_extending_expiry() -> Result<(), Error> {
        ...
        let (id, expires, count): (String, Option<i64>, i64) =
            sqlx::query_as("select id, expires, count(*) from async_sessions")
                .fetch_one(&mut store.connection().await?)
                .await?;

        assert_eq!(1, count);
        assert_eq!(expires.unwrap(), new_expires.unix_timestamp());
        assert_eq!(original_id, id);
    ...
    async fn creating_a_new_session_with_expiry() -> Result<(), Error> {
        ...
        assert_eq!(1, count);
        assert_eq!(id, cloned.id());
        assert!(expires.unwrap() > OffsetDateTime::now_utc().unix_timestamp());
